...
Let us say that `test_govcookiecutter/__init__.py` has functions called `hello` and `world` imported
into it, and both have docstrings. To automatically generate docstring documentation,
create a ReST file, and add the following line to reference the `test_govcookiecutter` module:

```rest
.. currentmodule:: test_govcookiecutter
```
...
Let us say that `test_govcookiecutter/__init__.py` has functions called `hello` and `world` imported
into it, and both have docstrings. To automatically generate docstring documentation,
create a Markdown file, and add the following line to reference the `test_govcookiecutter` module:
