...
@pytest.fixture
def make_temporary_features_json(make_temporary_folders_and_files: Path, test_input_json: Dict,
                                 test_input_json_filename: str) -> Path:
...
args_test_parse_features_json_attributes = [
    (["d"], ["e"], ["a.txt", "b/c.txt"], ["c/f.txt"], ["A", True, ["a.txt", "b/c.txt", "d"]], "test_manifest.json"),
    (["d"], ["e"], ["a.txt", "b/c.txt"], ["c/f.txt"], ["A", False, ["a.txt", "b/c.txt", "d"]], "test_manifest.json"),
    (["1", "2"], ["3", "4"], ["a.txt", "1/b.txt", "2/c/d.txt"], ["4/e.txt"],
     ["B", True, ["1", "2", "a.txt", "1/b.txt", "2/c/d.txt"]], "example_manifest.json"),
    (["1", "2"], ["3", "4"], ["a.txt", "1/b.txt", "2/c/d.txt"], ["4/e.txt"],
     ["B", False, ["1", "2", "a.txt", "1/b.txt", "2/c/d.txt"]], "example_manifest.json"),
]
args_test_parse_features_json = [
    (*a[:4], {"features": [dict(zip(JSON_BASE_KEYS, a[4]))]}, a[5]) for a in args_test_parse_features_json_attributes
]
...
class TestParseFeaturesJson:
    ...
    def test_json_load_called_correctly(self, mocker, make_temporary_features_json: Path) -> None:
    ...
    def test_returns_correctly(self, make_temporary_features_json: Path, test_input_folder_names: List[str],
                               test_input_filenames: List[str], test_input_json: Dict) -> None:
        """Test that the function returns as expected."""

        # Generate the expected outputs, execute the function, and test it return as expected, if the `remove` key is
        # True
        if test_input_json["features"][0]["remove"]:
            test_expected = [Path(p).resolve() for p in [*test_input_folder_names, *test_input_filenames]]
